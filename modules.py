...
def filter_blacklist(packet : Packet, blacklist_ref : db.Reference, iface : str):
    ...
    if packet.haslayer(IP) and packet.haslayer(TCP):
        ...
        if src_ip.replace('.', '-') in blacklist.keys() and tcp_field.flags == 0x02:
            os.system("iptables -A INPUT -p tcp -i %s -s %s --dport %d -j DROP" % (iface, src_ip, target_port))
        if src_ip.replace('.', '-') not in blacklist.keys() and tcp_field.flags == 0x02:
            os.system("iptables -A INPUT -p tcp -i %s -s %s --dport %d -j ACCEPT" % (iface, src_ip, target_port))
        ...
        if src_ip.replace('.', '-') in blacklist.keys():
            os.system("iptables -A INPUT -p udp -i %s -s %s --dport %d -j DROP" % (iface, src_ip, target_port))
        if src_ip.replace('.', '-') not in blacklist.keys():
            os.system("iptables -A INPUT -p udp -i %s -s %s --dport %d -j ACCEPT" % (iface, src_ip, target_port))
